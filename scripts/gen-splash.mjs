...
// Katalog v4 (zmiana koloru ciemnego tła): iOS zapamiętuje ekrany startowe przy dodaniu aplikacji do ekranu głównego; nowa ścieżka pozwala je podmienić.
...
await mkdir('public/splash/v4', { recursive: true })
const links = []
...
for (const [w, h, r] of DEVICES) {
  for (const t of THEMES) {
    ...
    const icon = await sharp(logo, { density: 384 })
      ...
      .png()
      .toBuffer()
    const file = `splash/v4/${w}x${h}@${r}-${t.name}.png`

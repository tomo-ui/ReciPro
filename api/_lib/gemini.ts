...
- ingredients: one entry per ingredient. ALWAYS write the quantity first, then the ingredient: "200 g mąki", "250 ml mleka", "2 jajka". Give solid and liquid amounts in metric units (g, kg, ml, l), converting cups, spoons and fractions ("1/2 szklanki mleka" -> "125 ml mleka", "pół szklanki mąki" -> "70 g mąki") when the text gives no metric amount; when it gives both, keep only the metric one. Never write fractions like 1/2 or words like "pół": use metric units or decimals ("0,5"). Keep countable items as plain numbers ("3 ząbki czosnku"). If no quantity is stated ("sól do smaku"), keep the text as is. Use "group" only for section headings such as "Ciasto" or "Sos".
